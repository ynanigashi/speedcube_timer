...
└─────────────────────────────────────────────────────────────┘
          ...
          (開始)  │            │ (アルゴリズム変更)
                 ...
                 ↓            │            ↓
      ┌───────────────────┐  │  ┌─────────────────────────────────────┐
      │ 前回選択または     │  │  │   PATTERN_ALGORITHM_SELECT 状態      │
      │ デフォルトの       │  │  │     (アルゴリズム選択画面)            │
      │ アルゴリズムを使用 │  │  │                                     │
      └───────────────────┘  │  │  ====== ALGORITHM: PLL Ua ======   │
                 │            │  │                                     │
...
└─────────────────────────────────────────────────────────────┘
          ...
          (次へ)  │   (再実行)  │   (評価保存) │    (終了)  │
                 ↓            ↓            ↓             ↓
        PATTERN_LIST_  PATTERN_READY  ┌─────────┐   READY
          SELECT              │        │ user_   │    (メイン
          (一覧に戻る)  (同じパターン) │ algorithm│    メニュー)
                              │        │ _ratings│
                              │        │ に保存   │
                              │        └─────────┘
                              │                │
...
**1. パターン選択時**
```
patterns.json → PatternDatabase → PATTERN_LIST_SELECT 画面
```

**2. アルゴリズム選択時**
```
algorithms.json → PatternDatabase.get_algorithms_for_pattern()
                ↓
         PATTERN_ALGORITHM_SELECT 画面
                ↓
    PatternAlgorithmSelectHandler (ENTER時)
                ↓
    stats.set_user_selected_algorithm(pattern_id, algorithm_id)
                ↓
    user_pattern_preferences テーブルに保存
```

**3. タイム記録時**
```
PatternRunningHandler (SPACE停止時)
        ↓
log_handler.save_pattern_solve(
    pattern_id="PLL_Ua",
    pattern_name="PLL Ua",
    pattern_category="PLL",
    solve_time=2.15,
    algorithm_id="PLL_Ua_standard",  ← 選択したアルゴリズムID
    practice_mode="manual",
    session_id=current_session_id
)
        ↓
pattern_solves テーブルに INSERT
```

**4. 評価保存時**
```
PatternFinishHandler (S キー押下時)
        ↓
stats.set_algorithm_rating(
    algorithm_id="PLL_Ua_standard",
    speed_rating=4,
    ergonomics_rating=5,
    is_favorite=True,
    user_notes=""
        ...
        ↓
user_algorithm_ratings テーブルに UPSERT
        ↓
stats.increment_algorithm_practice_count(algorithm_id)
        ↓
user_algorithm_ratings.practice_count += 1
```
